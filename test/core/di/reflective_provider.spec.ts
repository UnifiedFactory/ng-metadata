...
import { getInjectableName, provide } from '../../../src/core/di/provider';
import { createProvider } from '../../../src/core/di/provider_util';
import {
  resolveReflectiveProvider,
  _getAngular1ModuleMetadataByType,
  _isTypeRegistered,
  _normalizeProviders,
  _registerTypeProvider
} from '../../../src/core/di/reflective_provider';
...
describe(`di/reflective_provider`, () => {
  ...
  describe(`#_normalizeProviders`, () => {
    ...
    it(`should register $provide via service if provider is Decorated Type`, () => {
      ...
      expect(_isTypeRegistered(getInjectableName(HelloSvc), updatedAngular1Module, '$provide', 'service')).to.equal(
        true
      );
    });
    it(`should register $compileProvider via directive if provider is Decorated Type`, () => {
      ...
      expect(
        _isTypeRegistered(getInjectableName(HelloAttrDirective), updatedAngular1Module, '$compileProvider', 'directive')
      ).to.equal(true);

      expect(
        _isTypeRegistered(getInjectableName(MyComponent), updatedAngular1Module, '$compileProvider', 'directive')
      ).to.equal(true);
    });
    it(`should register $filterProvider via register if provider is Decorated Type`, () => {
      ...
      expect(
        _isTypeRegistered(getInjectableName(UpsHelloPipe), updatedAngular1Module, '$filterProvider', 'register')
      ).to.equal(true);
    });
    it(`should register config initializer if provider is class/function without Decorator`, () => {
      ...
      const updatedAngular1Module = _normalizeProviders(angular1Module, [
        stateConfig,
